...
async function setupI18n(options = { legacy: false, locale: DEFAULT_LOCALE, fallbackLocale: DEFAULT_LOCALE, messages: {} }) : Promise<I18n> {
...
async function loadLocaleMessages(i18n: I18n, locale: string) {
  ...
  const moduleMessages = await import(`../locales/${locale}_module_info`);
  const interfacesMessages = await import(`../locales/${locale}_interfaces_list`);
 ...
 */
export function tc(key: string, ...args: any[]): unknown {
  return computed(() => i18n.global.t(key, ...args));
}
