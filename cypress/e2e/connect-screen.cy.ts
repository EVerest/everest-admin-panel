...
describe("Connect-Screen", () => {
  ...
  it("should be possible to create, edit and delete an Everest instance", () => {
    ...
    cy.get('[data-cy="instance-name-field"]').type(
      `{selectall}{backspace}${newInstanceName}`,
    );
    cy.get('[data-cy="save-instance"]').click();
    cy.get('[data-cy="server-list-item"]')
  ...
  it("should automatically connect if autoConnect is set to true", () => {
    ...
    cy.get('[data-cy="server-list-item"]', { timeout: 6000 })
      .contains("Simulator")
      .should("be.visible");
    cy.wait(1000);
    cy.get('[data-cy="server-list-item"]')
    ...
    cy.visit("/");
    cy.get('[data-cy="hamburger-menu"]', { timeout: 6000 }).should(
      "be.visible",
    );
  });
});
