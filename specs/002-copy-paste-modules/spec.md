...
- **FR-009**: Pasted modules MUST be positioned at a fixed offset (exactly +20px X, +20px Y) from the original source position.
...
- **FR-012**: When multiple modules are selected, the right bar MUST display a summary state (e.g. 'N items selected') and disable single-module property editing.
