...
1. **Given** a selected module "Node", **When** user copies and pastes, **Then** a new module "Node (1)" is created.
2. **Given** a selected module "Node (1)", **When** user copies and pastes, **Then** a new module is created with name "Node (2)" (Smart Increment strategy).
3. **Given** multiple selected modules with connections between them, **When** copied and pasted, **Then** the new modules are connected to each other, but not to the originals or other modules.
4. **Given** a clipboard with modules, **When** user pastes, **Then** the new modules appear at a slight offset (e.g. +20px) from the original position.
...
- **FR-005**: System MUST support Copy (Cmd/Ctrl+C) to store selected modules and their internal connections in memory.
